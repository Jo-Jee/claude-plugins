...
claude-plugins — A knowledge base based on Karpathy's LLM Wiki pattern. The LLM reads original sources and incrementally builds and maintains a structured Markdown wiki. Unlike RAG, which re-extracts information every time, the goal here is to compile and accumulate knowledge.
